...
class TradingViewMCPAgentStyleTester:
    ...
    async def test_single_chart(self, ticker: str, interval: str = "240") -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, str):
                ...
                if "error" in result:
                    ...
                    if image_url.startswith("data:image/"):
                        base64_part = image_url.split(',', 1)[1] if ',' in image_url else ''
                        image_size = len(base64_part)
            ...
            if success:
                logger.info(f"✅ {ticker} ({interval}): {duration:.2f}s, {image_size:,} chars/bytes")
            else:
    ...
    async def run_performance_test(self, ticker: str, interval: str, runs: int) -> Dict[str, Any]:
        """Run multiple performance tests and calculate statistics"""
        logger.info(f"🚀 Starting agent-style performance test: {runs} runs of {ticker} ({interval})")
    ...
    async def run_concurrent_test(self, ticker: str, interval: str, concurrent_requests: int) -> Dict[str, Any]:
        """Run concurrent requests to test server performance under load"""
        logger.info(f"🚀 Starting concurrent test: {concurrent_requests} simultaneous requests for {ticker} ({interval})")
        ...
        if successful_results:
            durations = [r['duration'] for r in successful_results]
            image_sizes = [r['image_size'] for r in successful_results if r.get('image_size', 0) > 0]
            ...
            stats = {
                ...
                'std_request_duration': statistics.stdev(durations) if len(durations) > 1 else 0,
...
def print_performance_report(stats: Dict[str, Any]):
    ...
    if stats['avg_image_size'] > 0:
        if stats['avg_image_size'] > 10000:  # Likely base64 data
            print(f"   Average Size: {stats['avg_image_size']:,.0f} chars ({stats['avg_image_size']/1024:.1f} KB base64)")
        else:  # Likely URL length
            print(f"   Average Size: {stats['avg_image_size']:,.0f} chars (URL length)")
        print(f"   Size Range:   {stats['min_image_size']:,} - {stats['max_image_size']:,} chars")
    else:
...
def print_concurrent_report(stats: Dict[str, Any]):
    ...
    if stats['successful_requests'] == 0:
        ...
        if stats.get('failed_results'):
            ...
            for result in stats['failed_results'][:5]:  # Show first 5 failures
                print(f"   Request {result.get('request_id', '?')}: {result.get('error', 'Unknown error')}")
        return
    ...
    if stats['concurrent_requests'] > 1:
        speedup = stats['concurrent_requests'] / stats['total_duration'] * stats['avg_request_duration']
...
async def main():
    parser = argparse.ArgumentParser(description='Test TradingView MCP Performance using Agent Style')
    parser.add_argument('--ticker', default='BYBIT:BTCUSDT.P', help='Ticker symbol to test')
    parser.add_argument('--interval', default='240', help='Chart interval (240, 15, D, etc.)')
    parser.add_argument('--runs', type=int, default=5, help='Number of test runs (for sequential testing)')
    parser.add_argument('--concurrent', type=int, help='Number of concurrent requests (enables concurrent mode)')
    ...
    try:
        if args.concurrent:
            # Run concurrent test
            stats = await tester.run_concurrent_test(args.ticker, args.interval, args.concurrent)
            ...
            # Run sequential test
            stats = await tester.run_performance_test(args.ticker, args.interval, args.runs)
