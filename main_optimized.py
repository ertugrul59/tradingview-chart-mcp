...
def validate_environment(require_auth=True):
    ...
    if not session_id or not session_id_sign:
        logger.error("Error: TRADINGVIEW_SESSION_ID and TRADINGVIEW_SESSION_ID_SIGN must be set when --auth is enabled.")
        logger.error("       Provide them either via environment variables (e.g., in MCP client config)")
        logger.error("       or in a .env file in the project directory for local execution.")
        sys.exit(1)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="TradingView Chart MCP Server - Optimized for Concurrent Performance")
    ...
    parser.add_argument("--auth", action="store_true", help="Enable authentication (default: True for TradingView)")
    parser.add_argument("--max-concurrent", type=int, default=4, 
                       help="Maximum concurrent requests (default: 4, based on performance testing)")
    parser.add_argument("--disable-pooling", action="store_true", 
...
class OptimizedTradingViewMCPServer:
    ...
    def _initialize_browser_pool(self):
        ...
        self.logger.info(f"🔥 Initializing optimized browser pool with {self.max_concurrent} instances...")
        ...
        for i in range(self.max_concurrent):
            try:
                ...
                scraper = TradingViewScraper(
                    ...
                    chart_page_id=self.config.get('chart_page_id') or TradingViewScraper.DEFAULT_CHART_PAGE_ID,
                ...
                raise TradingViewScraperError(f"Browser pool initialization failed: {e}")
        
        self.logger.info(f"✅ Browser pool initialized! Ready for {self.max_concurrent} concurrent requests")
        self.logger.info(f"🚀 Expected performance: ~2.5-3.5s per request for 3 concurrent (70-80% faster than baseline)")
    ...
    async def get_chart_optimized(self, ticker: str, interval: str) -> str:
        ...
        async with self.semaphore:  # Limit concurrent requests to optimal level
            ...
            try:
                ...
                self.logger.debug(f"📊 Processing {ticker} ({interval}) with pooled browser")
                
                # Use existing browser - much faster than creating new one
                result = await asyncio.get_event_loop().run_in_executor(
                    None, 
                    scraper.get_chart_image_url,
                    ticker,
                    interval
                )
                ...
                self.logger.info(f"✅ Chart completed for {ticker} in {duration:.2f}s (avg: {avg_time:.2f}s)")
                
                return result
                
            except Exception as e:
                duration = time.time() - start_time
                self.logger.error(f"❌ Chart request failed for {ticker} after {duration:.2f}s: {e}")
...
if not args.disable_pooling:
    logger.info(f"🚀 Initializing OPTIMIZED TradingView MCP server with browser pooling ({args.max_concurrent} concurrent)")
    optimized_server = OptimizedTradingViewMCPServer(max_concurrent=args.max_concurrent, config=config)
else:
...
async def get_tradingview_chart_image(ticker: str, interval: str, ctx: Context) -> str:
    ...
    """
    await ctx.info(f"🔍 [OPTIMIZED] Getting TradingView chart for {ticker} interval {interval}")
    
    try:
        if optimized_server and not args.disable_pooling:
            # Use optimized server with browser pooling
            image_url = await optimized_server.get_chart_optimized(ticker=ticker, interval=interval)
        else:
            ...
            with TradingViewScraper(
                ...
                if config['use_save_shortcut']:
                    image_url = scraper.get_chart_image_url(ticker=ticker, interval=interval)
                    if not image_url:
                        raise TradingViewScraperError("Scraper did not return an image URL.")
                else:
                    screenshot_link = scraper.get_screenshot_link(ticker=ticker, interval=interval)
        ...
        raise Error(code=500, message="An unexpected error occurred while fetching the TradingView chart image.")
...
async def get_chart_prompt_generic(ticker: str, interval: str, ctx: Context):
    await ctx.info(f"🔍 [OPTIMIZED] Executing prompt: Get the {interval} chart for {ticker}")
    interval_map = {
        "1 minute": "1", "5 minute": "5", "15 minute": "15", "30 minute": "30",
        "1 hour": "60", "4 hour": "240", "daily": "D", "weekly": "W", "monthly": "M"
    }
...
async def get_chart_prompt_daily_tradingview(ticker: str, ctx: Context):
    await ctx.info(f"🔍 [OPTIMIZED] Executing prompt: Show me the daily TradingView chart for {ticker}")
...
async def get_chart_prompt_timeframe(ticker: str, interval: str, ctx: Context):
    await ctx.info(f"🔍 [OPTIMIZED] Executing prompt: Fetch TradingView chart image for {ticker} on the {interval} timeframe")
    interval_map = {
...
if __name__ == "__main__":
    if os.getenv("MCP_DEBUG_STARTUP", "false").lower() == "true":
        ...
        logger.info(f"   - Browser Pooling: {'Enabled' if not args.disable_pooling else 'DISABLED'}")
    ...
    try:
        if args.transport == "streamable-http":
            logger.info(f"Starting optimized TradingView MCP server on {args.host}:{args.port}")
